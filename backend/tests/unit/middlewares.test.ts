...
import { sanitizeHeaders, requireJsonContentType, sanitizeForLlm } from '../../src/middlewares/sanitize';
...
describe('sanitizeHeaders', () => {
  ...
  it('should reject headers containing null bytes', () => {
    ...
    const { res, status } = mockRes(); const next = mockNext();
    sanitizeHeaders(req, res, next);
...
describe('requireJsonContentType', () => {
  ...
  it('should allow application/json; charset=utf-8', () => {
    const req = mockReq({ headers: { 'content-type': 'application/json; charset=utf-8' } });
    const { res } = mockRes(); const next = mockNext();
    requireJsonContentType(req, res, next);
    expect(next).toHaveBeenCalled();
  });
...
describe('sanitizeForLlm', () => {
  ...
  it('should preserve normal PR title text', () => {
    expect(sanitizeForLlm('feat: add user authentication')).toBe('feat: add user authentication');
    expect(sanitizeForLlm('fix: resolve null pointer in parser')).toBe('fix: resolve null pointer in parser');
  });
...
describe('replayProtectionMiddleware', () => {
  ...
  it('should treat different delivery IDs independently', () => {
    ...
    const { res } = mockRes();
    const next1 = mockNext(); const next2 = mockNext();
