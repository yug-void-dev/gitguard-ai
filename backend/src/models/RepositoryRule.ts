 ...
 */
export function evaluateFindingAgainstSpec(
  spec: IRepositoryRuleSpec,
  finding: { severity: string; confidence: number; category?: string; file?: string },
): boolean {
  if (!spec) return true;
...
// --- Model export ---
export const RepositoryRule: Model<IRepositoryRuleDoc> =
  mongoose.model<IRepositoryRuleDoc>('RepositoryRule', repositoryRuleSchema);
