...
export function processDiff(
  ...
  log.info(
    { files: processedFiles.length, skipped: skippedFiles.length, chunks: chunks.length },
...
function parseAndClean(rawDiff: string): {
...
function cleanBlock(lines: string[]): {
  cleanDiff: string;
  additions: number;
  deletions: number;
  contextLines: number;
...
function buildChunk(
  ...
  const fileList = files
    .map(
      (f) =>
        `  • ${f.filename} [${f.language}] ${f.changeType} +${f.additions}/-${f.deletions}`,
    )
    .join('\n');
  const contextHeader = [
